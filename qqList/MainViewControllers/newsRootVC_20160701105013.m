...
//        if (self.FuncselectSegm.selectedSegmentIndex ==0) {
//            [[self.observerModel mutableArrayValueForKey:@"newsArray" ] removeObjectAtIndex:indexPath.row];
...
{
    UITableViewRowAction *action1 = [UITableViewRowAction  rowActionWithStyle:UITableViewRowActionStyleNormal title:@"加入黑名单" handler:^(UITableViewRowAction * _Nonnull action, NSIndexPath * _Nonnull indexPath) {
        //在block中实现相对应的事件
...
//    2、当左滑按钮执行的操作涉及数据源和页面的更新时，要先更新数据源，在更新视图，否则会出现无响应的情况
    UITableViewRowAction *toTop = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDestructive title:@"置顶"handler:^(UITableViewRowAction * _Nonnull action, NSIndexPath * _Nonnull indexPath) {
        //更新数据（此操作不会触发贯观察者）
        if (self.FuncselectSegm.selectedSegmentIndex ==0) {
            [self.observerModel.newsArray  exchangeObjectAtIndex:indexPath.row withObjectAtIndex:0];
        }else{
            [self.phoneArray exchangeObjectAtIndex:indexPath.row withObjectAtIndex:0];
        }
        //更新页面
